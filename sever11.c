/****SERVER CODE****/  
 ...
 #include <sys/types.h>            // include various data types      
 ...
 #define PORT 4444                //default port number  
...
struct Node                         /*structure to handle all clients*/  
   ...
   typedef ptrtonode addr1;  
      ...
      void Insert(int port,char*,head h,addr1 a);/*inserting new client */  
...
int main(int argc, char *argv[]) {  
	...
	int      portnum;               		/*variable for port numb if provided*/  
        ...
        struct sockaddr_in addr,cl_addr;    		/*structure to hold client's address */  

        char clientaddr[CLADDR_LEN];  
        int      cli_size,z;               		/*length of address */  
        pthread_t      thr;              		/*variable to hold thread ID */  
        int      yes=1;  
        addr1 a;                         		/*variable of type struct addr*/  
...
/*=       set info of server       =*/  
	...
	cl_addr.sin_family=AF_INET;                   /* set family to Internet   */  
        cl_addr.sin_addr.s_addr = INADDR_ANY;         /* set IP address */  
        cl_addr.sin_port=PORT;  
        printf("IP ADDRESS:\t%s\n",inet_ntoa(server_addr.sin_addr));   /* returns the dots-and-numbers string in a 
                                                                          static buffer that overwritten with each call to the function.*/
...
/*=       creating socket         =*/
	...
	if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &yes,sizeof(int)) == -1) {   //set the socket options
...
while(1){  
	...
	cli_size=sizeof(cl_addr); //cli_size necessary as an argument for pthread_create  
        new_fd = accept(sockfd, (struct sockaddr *) &cl_addr,&cli_size); //accepting connection from client   
        a =h ;  
        
	   /*=sign in with name=*/  

	bzero(username,10);       // initializes the buffer using the bzero()      
  ...
  /*=notify all clients about newly joining clients=*/   
           ...
           send(sf2,buffer ,sizeof(buffer),0);  
             ...
             //printf("server got connection from %s & %d\n\n",inet_ntoa(cl_addr.sin_addr),clientaddr,CLADDR_LEN); // debugging
...
/********main ends***********/  
 ...
 void Insert( int port,char *username, head h, addr1 a )  
      {  
      addr1 TmpCell;  
